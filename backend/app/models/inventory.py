...
from sqlalchemy import Computed, DateTime, ForeignKey, Integer, Numeric, String, Text, UniqueConstraint, func
...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(String(40), default=UserRole.consulta.value)
...
class Product(Base):
    ...
    units_per_case: Mapped[int] = mapped_column(Integer, default=12)
    is_active: Mapped[bool] = mapped_column(default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class StockPosition(Base):
    ...
    physical_confirmed: Mapped[int] = mapped_column(Integer, default=0)
    reserved: Mapped[int] = mapped_column(Integer, default=0)
    invoiced_pending_dispatch: Mapped[int] = mapped_column(Integer, default=0)
    blocked_incident: Mapped[int] = mapped_column(Integer, default=0)
    incoming_expected: Mapped[int] = mapped_column(Integer, default=0)
...
class PurchaseOrder(Base):
    ...
    status: Mapped[str] = mapped_column(String(60), default=PurchaseOrderStatus.recibida.value)
...
class Reservation(Base):
    ...
    quantity: Mapped[int] = mapped_column(Integer)
    status: Mapped[str] = mapped_column(String(60), default=ReservationStatus.activa.value)
    reason: Mapped[str] = mapped_column(Text)
    created_by_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"))
    released_by_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"))
...
class Invoice(Base):
    ...
    customer_name: Mapped[Optional[str]] = mapped_column(String(255))
    purchase_order_id: Mapped[Optional[int]] = mapped_column(ForeignKey("purchase_orders.id"))
    status: Mapped[str] = mapped_column(String(60), default=InvoiceStatus.facturada.value)
    contifico_source_id: Mapped[Optional[str]] = mapped_column(String(120))
    registered_by_user_id: Mapped[Optional[int]] = mapped_column(ForeignKey("users.id"))
...
class Incident(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    status: Mapped[str] = mapped_column(String(60), default=IncidentStatus.abierta.value)
...
class Approval(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    status: Mapped[str] = mapped_column(String(60), default=ApprovalStatus.solicitada.value)
